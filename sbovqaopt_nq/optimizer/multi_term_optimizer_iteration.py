'''
Defines the MultiTermOptimizerIteration class.
'''
import numpy as np
...
class MultiTermOptimizerIteration:
    '''
    Implements a single iteration of surrogate-based optimization using
    a Gaussian kernel for cost functions comprised of multiple weighted
    terms.
    '''
    ...
    def minimize_kde(
        ...
        measured_values = np.atleast_2d(
            [term(x) for x in training_point_angles]
        ).T
